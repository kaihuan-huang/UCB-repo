...
app.get('/api/categories', (req, res) => {
  const categories = termData.map((term) => term.category); //category array

  const result = categories.filter((cat, i) => categories.indexOf(cat) === i);  //index cat in category array
  console.log(categories.indexOf(cat) === i);
  return res.json(result);
});
